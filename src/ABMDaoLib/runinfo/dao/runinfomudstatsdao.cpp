#include "runinfomudstatsdao.h"
#include "../../abmdaolib.h"

RunInfoMudStatsDao::RunInfoMudStatsDao()
{

}

bool RunInfoMudStatsDao::add(QSharedPointer<RunInfoMudStats> entity)
{
...
bool RunInfoMudStatsDao::updateById(QSharedPointer<RunInfoMudStats> entity)
{
  if(entity->getId() == 0){
    qDebug() << "id 不能为0";
    return false;
...
QList<QSharedPointer<RunInfoMudStats> > RunInfoMudStatsDao::findList(QVector<SqlCondition> &conditions)
{
  QList<QMap<QString,QVariant>> dataList;
  ABMDaoLib::getInstance()->getSqlUtils()->queryDatas(m_tableName,conditions,dataList);

  QList<QSharedPointer<RunInfoMudStats>> list;
  for(int i=0;i<dataList.size();i++){
    QSharedPointer<RunInfoMudStats> entity = QSharedPointer<RunInfoMudStats>(new RunInfoMudStats());
...
RunInfoMudStatsPage RunInfoMudStatsDao::findPage(QVector<SqlCondition> &conditions, const int pageSize, const int pageNum)
{
  RunInfoMudStatsPage page;
  SqlPage sqlPage;
...
QSharedPointer<RunInfoMudStats> RunInfoMudStatsDao::findById(int id)
{
  QVector<SqlCondition> conditions;
...
QSharedPointer<RunInfoMudStats> RunInfoMudStatsDao::findByRunInfoId(int runInfoId)
{
  QVector<SqlCondition> conditions;
